...
model_layout_flags = ['num_layers', 'meta_layer_size', 'hidden_size', 'biscale_slow_layer_ticks', 'multiscale', 'variable_ticks', 'multiscale', 'feed_previous', 'pace_events']
...
class RNNGAN(object):
  ...
  def __init__(self, is_training, num_song_features=None, num_meta_features=None):
    ...
    with tf.variable_scope('G') as scope:
      ...
      length_freq_velocity = tf.random_uniform(shape=[batch_size, num_song_features], minval=0.0, maxval=1.0)
      outputs = []
  ...
  def discriminator(self, inputs, is_training):
    if FLAGS.conv_d:
      ...
      decision = tf.Print(decision, [decision],
              'decision = ', summarize=20, first_n=20)
      return decision
...
def main(_):
  ...
  loader = music_data_utils.MusicDataLoader(FLAGS.datadir, FLAGS.select_validation_percentage, FLAGS.select_test_percentage, FLAGS.variable_ticks, FLAGS.works_per_composer, FLAGS.pace_events)
